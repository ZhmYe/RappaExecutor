...
class BinomialDiffusionActive(BinomialDiffusionVanilla):
    ...
    # 计算q(xt-1|xt, x0)正比例于q(xt-1|xt)q(xt-1|x0) 但是这里进一步加入了degree变化的影响
    # 这里的后验概率变成了判断节点是否活跃
    def _q_posterior_actives(self, degree_start, degree_t, t_node):
        # t_node - 1，表示前一时刻
        tmin1 = t_node - 1
        # 如果tmin1小于0，将其设置为0，因为不存在负的时间步
        ...
        # 计算试验次数，即degree_start - degree_t，如果结果小于0，将其设置为0， degree变化的数量
    ...
    def _p_sample_and_set_actives(self, batched_graph, t_node):
        if self.parametrization == 'xt_prescribed_st':
            ...
            active_node_masks = self.log_sample_categorical(log_model_prob_active, num_classes=2).argmax(1).bool()
            batched_graph.active_node_indices = active_node_masks.nonzero(as_tuple=True)[0]
            batched_graph.active_edge_indices = active_node_masks[batched_graph.full_edge_index[0]].logical_and(active_node_masks[batched_graph.full_edge_index[1]]).nonzero(as_tuple=True)[0] 
    ...
    # 去噪 预测xt-1, 这当中会用到s_t    st := ✶[dt−1 = dt] is a binary vector indicating whether nodes are active (having degree change from t − 1 to t) or not from t − 1 to t. 
