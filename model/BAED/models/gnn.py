...
class BernNet(nn.Module):
    ...
    def forward(self, graph):
        ...
        in_feat = graph.ndata['feature'].to(dtype=torch.float)  # 转换为 float32
...
class AMNet(nn.Module):
    ...
    def forward(self, graph):
        ...
        y_hat = torch.max(y_hat,dim=0).values
        return y_hat
...
class GCN(nn.Module):
    ...
    def forward(self, graph):
        h = graph.ndata['feature'].to(dtype=torch.float)  # 转换为 float32
...
class GIN(nn.Module):
    def __init__(self, in_feats, h_feats=32, num_classes=2, num_layers=2, agg='mean', dropout_rate=0,
        ...
        self.layers = nn.ModuleList()
        self.act = getattr(nn, activation)()
        self.layers.append(dglnn.GINConv(nn.Linear(in_feats, h_feats), aggregator_type=agg))
        for i in range(1, num_layers-1):
            self.layers.append(dglnn.GINConv(nn.Linear(h_feats, h_feats), aggregator_type=agg))
        self.layers.append(dglnn.GINConv(nn.Linear(h_feats, num_classes), aggregator_type=agg))
        self.dropout = nn.Dropout(dropout_rate) if dropout_rate > 0 else nn.Identity()
...
class GraphSAGE(nn.Module):
    ...
    def forward(self, graph):
        # h = graph.ndata['feature']
        h=graph.ndata['feature'].to(dtype=torch.float)
