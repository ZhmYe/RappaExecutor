...
def calculate_fundamental_value(prices):
    ...
    smoothed_state_means, _ = kf.smooth(prices)
...
'''def compute_loss_gradients(true_prices, loss, params_index, config, fundamental_value, dates, trader_type, cof_hill, cof_vol, cof_acf, cof_square_acf, true_markets, freq):
    ...
    for index in params_index:
        ...
        simulated_prices_eps, simulated_markets_eps = simulate_market(traders, exchange, market, true_prices, dates, trader_type, config_eps)
...
# -------------- 参数校准函数 --------------------
def calibrate_parameters(initial, learning_rate=0.01, epsilon=0.001, epoch=100):
    """
    param:
        initial: 参数校准所需要的初始化数据，包括
            [true_prices: 历史价格; true_markets: 历史十档行情;
            cof_dict: 损失函数权重; fundamental_value: 基本价值量; dates: 时间戳; trader_type: 交易者类型;
            params_index: 需要更新的参数； config: 模型初始化参数集合]

        learning_rate: 学习率 (default: 0.01)
        epsilon: 有限差分步长 (default: 0.001)
        epoch: 校准迭代次数 (default: 100)
    return:
        optimized_params : 优化后的参数
        loss_history     : 损失值历史记录
    """
    true_prices = initial['true_prices']
    true_markets = initial['true_markets']
    cof_dict = initial['cof_dict']
    fundamental_value = initial['fundamental_value']
    dates = initial['dates']
    trader_type = initial['trader_type']
    params_index = initial['params_index']
    config = initial['config']
    seed_count = initial.get('seed_count', 1)
    base_seed = initial.get('base_seed', 2026)
    param_bounds = initial.get('param_bounds', {})
    patience = initial.get('patience', 0)

    print("Start Simulation and Training")
    gradients = {}
    loss_history = []
    ...
    def apply_bounds(config_dict):
        bounded = config_dict.copy()
        for key, (lower, upper) in param_bounds.items():
            if key in bounded:
                bounded[key] = max(lower, min(upper, bounded[key]))
        return bounded

    def evaluate_loss(config_dict):
        losses = []
        for seed_idx in range(seed_count):
            ...
            traders, exchange, market = create_instance(config_dict, fundamental_value, true_prices[0])
            simulated_prices, simulated_markets, _ = simulate_market(
                traders, exchange, market, true_prices, dates, trader_type, config_dict
            )
            cur_loss = loss_function(cof_dict, true_prices, simulated_prices, true_markets, simulated_markets)
            losses.append(cur_loss)
        return float(np.mean(losses))

    for i in range(epoch):
        current_loss = evaluate_loss(current_config)
        ...
        for index in params_index:
            ...
            config_plus[index] += epsilon
            config_plus = apply_bounds(config_plus)
            loss_plus = evaluate_loss(config_plus)

            config_minus = current_config.copy()
            config_minus[index] -= epsilon
            config_minus = apply_bounds(config_minus)
            loss_minus = evaluate_loss(config_minus)

            gradients[index] = (loss_plus - loss_minus) / (2 * epsilon)

        for index in params_index:
            current_config[index] -= learning_rate * gradients[index]
        current_config = apply_bounds(current_config)

        if i % 10 == 0:
            print(f"step: {i}/{epoch}, loss: {current_loss:.4f}, best_loss: {best_loss:.4f}")

        if patience > 0 and no_improve_round >= patience:
            print(f"Early stopping at step {i}, no improvement in {patience} rounds.")
            break

    final_params = {}
    for index in params_index:
        final_params[index] = round(best_config[index], 4)

    print("Finish Simulation and Training")
    print("The Final update params is: ", final_params)
    print(f"Best loss: {best_loss:.6f}")

    return best_config, loss_history
