...
import grpc
import network.Grpc.services.services_pb2_grpc as pb2_grpc
import network.Grpc.services.services_pb2 as pb2
...
class GrpcEngine:
    ...
    class CoordinatorService(pb2_grpc.CoordinatorServicer):
        ...
        def Schedule(self, request, context):
            return self.grpc_engine.server_handle_schedule(request, handler)
    ...
    def server_handle_schedule(self, request, context) -> pb2.ScheduleResponse:
        if self.node_id in request.schedule:
            # 节点在其调度内，将任务加入当前任务的队列中
            new_task = PendingTaskPoolItem(request.sign, request.slot, request.schedule[self.node_id], request.model)
            self.pending_task_pool.put(new_task)
            log.write_log("DEBUG", f"receive Task {request.sign} Slot {request.slot}")
            return pb2.ScheduleResponse(accept=True, nodeId=self.node_id, sign=request.sign)
        else:
            # 不在调度内，则拒绝
            return pb2.ScheduleResponse(accept=False, nodeId=self.node_id, sign=request.sign,
