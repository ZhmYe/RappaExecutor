...
class SlotManager:
    def __init__(self, slot_channel: Queue=Queue()):
    ...
    def handle_receive_slot(self, slot: CommitSlotItem) -> int:
    ...
    def process(self):
        # 处理channel中的slot
        while True:
            if self.slot_channel.empty():
                continue
            try:
                slot: CommitSlotItem = self.slot_channel.get(timeout=1)  # Wait for a task
                if slot.index == -1:
                    raise ValueError("Slot put in slot_channel must satisfy index != -1")
                # todo 判断异常invalid
