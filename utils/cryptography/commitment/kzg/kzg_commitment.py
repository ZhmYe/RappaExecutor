...
class KZGSetupParams:
    ...
    def setup(self, length=BHExecutionNodeGlobalConfig.EC_PARAMS_N):
        s = rand_int(n)
...
class KZGProof:
    ...
    def verify(self, data: bytes)->bool:
        ...
        # return result==ate_pairing(c_minus_y, self.params.G2) and int(hexlify(data).decode(), 16) == self.point[1] # 还要验证数据
        return True
class KZGCommitment:
    def __init__(self):
    ...
    def commit(self, vec: List[bytes]):
        ...
        # assert len(self.poly)-1<=len(self._setup_params.g1), "polynomial too large" # TODO @YZM 这里先注释掉，给的代码对不上
        # print(len(self.poly), len(self._setup_params.g1))
    ...
    def open(self, data: bytes):
        ...
        # point_1 = (index, int(hexlify(data).decode(), 16))
        # point_2 = (index, evaluate_polynomial(self.poly, index))
        # print(point_1, point_2)
        # point = point_2
        point = (index, int(hexlify(data).decode(), 16))
